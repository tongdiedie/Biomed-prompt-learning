...
class CrossModalPromptFusion(nn.Module):
    ...
    def forward(self, visual_prompt, text_prompt):
        ...
        """
        B, num_v_tokens, dim = visual_prompt.shape
        _, num_t_tokens, _ = text_prompt.shape
        
        # ========== Step 1: 双向注意力交互 ==========
        # 转换为(seq_len, batch, dim)格式
        v = visual_prompt.transpose(0, 1)  # [num_v_tokens, B, dim]
        t = text_prompt.transpose(0, 1)    # [num_t_tokens, B, dim]
